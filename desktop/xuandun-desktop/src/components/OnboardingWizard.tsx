...
import {
  Plug, Package, FlaskConical, Rocket, Check, Lightbulb,
  CheckCircle, RefreshCw, BarChart3, Bell, FileText, type LucideIcon,
} from 'lucide-react';
...
const METHODS: Method[] = [
  {
    ...
    steps: [
      { title: '启动代理服务', detail: '在「设置 → 代理服务」卡片中点击「启动代理」，玄盾将在 127.0.0.1:18765 监听' },
      { title: '配置 AI 工具代理', detail: '打开 Claude Desktop / Cursor 的网络设置，将 HTTP 代理地址设为 127.0.0.1:18765' },
      { title: '或使用 MCP Server 模式', detail: '在 Claude Desktop 配置文件中添加 xuandun_protect 工具，实现更精细的拦截控制（详见文档）' },
  ...
  },
  {
    ...
    steps: [
      { title: '安装 SDK', detail: '执行 pip install daoti-xuandun==1.2.3 安装最新版 SDK' },
...
export default function OnboardingWizard({ totalRequests, engineRunning, onSkip, onNavigate }: OnboardingWizardProps) {
  ...
  // P1-19 修复：手动检查无流量时显示提示，避免用户感知按钮无响应
  ...
  const handleVerifyNow = () => {
    if (totalRequests > 0) {
      ...
      // P1-19 修复：无流量时给出明确提示，避免用户误以为按钮无响应
      setVerifyHint(
        '暂未检测到流量接入。请确认：\n' +
        '1. 代理服务已启动（设置 → 代理服务）\n' +
        '2. AI 工具代理地址已配置为 127.0.0.1:18765\n' +
        '3. 在 AI 工具中发送至少一条消息'
      );
      setTimeout(() => setVerifyHint(null), 5000);
    }
  };
      ...
      <div className="onboarding-wizard">
        <div className="wizard-header">
          <div className="wizard-title-row">
            <Rocket size={32} strokeWidth={1.5} className="wizard-icon" />
          ...
          <div className="wizard-steps-indicator">
            {WIZARD_STEPS.map((label, idx) => (
              ...
              >
                <span className="wizard-step-num">{idx < wizardStep ? <Check size={16} strokeWidth={1.5} /> : idx + 1}</span>
          ...
          {wizardStep === 0 && (
            <div className="wizard-step-content">
              <div className="wizard-observing-badge">
                <span className="wizard-badge-icon status-dot" style={{ background: 'var(--dt-warning)', boxShadow: '0 0 6px var(--dt-warning)' }}></span>
                <span>
              ...
              <div className="wizard-method-grid">
                {METHODS.map((m) => {
                  const Icon = ICON_MAP[m.icon] ?? Plug;
                  return (
                    <div
                      key={m.id}
                      className={`wizard-method-card ${selectedMethod === m.id ? 'selected' : ''}`}
                      onClick={() => handleSelectMethod(m.id)}
                      // P1-20 修复：可点击 div 添加键盘可访问性，Tab 聚焦 + Enter/Space 触发
                      onKeyDown={(e) => {
                    ...
                    >
                      <div className="wizard-method-icon"><Icon size={28} strokeWidth={1.5} /></div>
          ...
          {wizardStep === 1 && method && (
            <div className="wizard-step-content">
              <div className="wizard-method-header">
                {(() => {
                  const Icon = ICON_MAP[method.icon] ?? Plug;
                  return <span className="wizard-method-icon-lg"><Icon size={32} strokeWidth={1.5} /></span>;
                })()}
                <div>
              ...
              <div className="wizard-checklist">
                {method.steps.map((step, idx) => (
                  <div
                    ...
                    tabIndex={0}
                    role="button"
                    aria-pressed={!!checkedSteps[idx]}
                  >
                    <div className={`wizard-checkbox ${checkedSteps[idx] ? 'checked' : ''}`}>
                      {checkedSteps[idx] ? <Check size={14} strokeWidth={1.5} /> : ''}
                    </div>
              ...
              {!allStepsChecked && (
                <div className="wizard-hint-bar">
                  <Lightbulb size={16} strokeWidth={1.5} /> 勾选所有步骤后可进入下一步验证连通性
                </div>
              )}
            </div>
          )}
              ...
              <div className="wizard-verify-section">
                {totalRequests > 0 ? (
                  <div className="wizard-verify-success">
                    <div className="wizard-verify-icon"><CheckCircle size={24} strokeWidth={1.5} /></div>
                    <h3>已检测到流量接入！</h3>
                    <p>玄盾已成功检测到 {totalRequests} 条请求，接入配置正确。</p>
                  </div>
                ) : (
                  <div className="wizard-verify-waiting">
                    <div className="wizard-verify-icon spinning"><RefreshCw size={24} strokeWidth={1.5} /></div>
                    <h3>正在等待流量接入...</h3>
                    <p>
                    ...
                    {method?.id === 'proxy' && (
                      <div className="wizard-verify-tip">
                        提示：确认代理已启动（设置 → 代理服务 → 启动代理），且 AI 工具代理地址指向 127.0.0.1:18765
                      </div>
                    )}
                    {method?.id === 'sdk' && (
                      <div className="wizard-verify-tip">
                    ...
                    {/* P1-19 修复：无流量时显示提示，告知用户排查方向 */}
                    {verifyHint && (
                      <div className="wizard-verify-hint" style={{
                        marginTop: '12px', padding: '8px 12px',
                        background: 'var(--bg-secondary)', borderRadius: '6px',
                        fontSize: '0.85em', whiteSpace: 'pre-line', color: 'var(--text-primary)',
                      }}>
                        {verifyHint}
                      </div>
                    )}
          ...
          {wizardStep === 3 && (
            <div className="wizard-step-content">
              <div className="wizard-complete">
                <div className="wizard-complete-icon"><CheckCircle size={32} strokeWidth={1.5} /></div>
                <h2>接入完成！</h2>
                <p>
                ...
                <div className="wizard-complete-tips">
                  <div className="wizard-tip-item">
                    <span className="wizard-tip-icon"><BarChart3 size={16} strokeWidth={1.5} /></span>
                    <span>Dashboard 提供趋势图、攻击分布和实时监控</span>
                  </div>
                  <div className="wizard-tip-item">
                    <span className="wizard-tip-icon"><Bell size={16} strokeWidth={1.5} /></span>
                    <span>在「设置 → 告警通道」配置钉钉/飞书/邮件告警</span>
                  </div>
                  <div className="wizard-tip-item">
                    <span className="wizard-tip-icon"><FileText size={16} strokeWidth={1.5} /></span>
