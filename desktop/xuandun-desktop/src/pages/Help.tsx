...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    icon: Plug,
    title: '上游模型配置',
    desc: '玄盾作为防护网关保护你的大模型。首次使用前，需要在这里告诉玄盾你的大模型服务地址（OpenAI 兼容接口）。配置完全可视化，无需修改代码或设置系统环境变量。',
    steps: [
      { p: '进入「系统设置 → 上游模型」。' },
      { p: '「上游模型地址」填写你的模型服务地址（OpenAI 兼容，如 https://api.openai.com/v1），必填。' },
      { p: '「API Key」填写鉴权密钥；若你的模型是私有化部署、无需鉴权，可留空。' },
      { p: '「默认模型名」可填（如 gpt-4o）；留空时使用每次请求里指定的模型名。' },
      { p: '「超时时间」按需调整请求上游的等待秒数（默认 300 秒）。' },
      { p: '点「保存」，然后到「引擎管理」卡片点「重启引擎」（或重启玄盾应用），配置即生效。' },
    ],
    consequences: [
      { h: '保存后需重启引擎才生效', p: '配置写入本地后，要重启引擎（专家模式 → 引擎管理 → 重启，或重启玄盾应用）才会注入生效。' },
      { h: '地址必须以 http:// 或 https:// 开头', p: '填写时若省略协议头会保存失败，请完整填写，例如 https://api.openai.com/v1。' },
      { h: '未配置时调用会报 503', p: '若未配置上游模型地址，发起 /v1/chat/completions 请求会返回 "XuanDun gateway not configured"（503）。配置并重启引擎后即可正常。' },
    ],
    faq: [
      { q: '我的模型需要 API Key 吗？', a: '云端服务（如 OpenAI、通义、文心等）一般需要；私有化/本地部署的大模型通常不需要，此时 API Key 留空即可。' },
      { q: '支持多个模型吗？', a: '当前版本是单模型防护网关，一次只连接一个上游模型。如需切换，修改配置后重启引擎即可。' },
    ],
  },
  {
    id: 'mode',
