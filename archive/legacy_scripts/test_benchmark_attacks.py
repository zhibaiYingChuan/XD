...
print(f'HIGH: <= {dt*0.6:.4f}, MEDIUM: <= {dt*1.8:.4f}, LOW: <= {rb:.4f}')
print(f'Profile: {da._domain_char_profile}')
print()

datasets = build_all_datasets()
for ds in datasets:
    if ds.category in ('prompt_injection', 'jailbreak', 'prompt_leaking', 'chinese_attack', 'cybersec', 'adversarial', 'multilingual', 'role_play'):
        print(f'=== {ds.name} ===')
        for text, label in ds.samples[:5]:
            if label == 'attack':
                ...
                print(f'  dist={r.domain_distance:.4f} anomaly={anomaly:.4f} → {status} | {text[:50]}')
        print()